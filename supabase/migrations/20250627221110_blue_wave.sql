-- Insert doctors with Tavus integration (only if they don't already exist)
...
) AS new_doctors(name, specialty, description, icon, tavus_replica_id, tavus_persona_id, is_premium, consultation_price)
WHERE NOT EXISTS (
  SELECT 1 FROM doctors WHERE tavus_replica_id = new_doctors.tavus_replica_id
);

-- Insert comprehensive medicine and food recommendations (only if they don't already exist)
INSERT INTO recommendations (name, category, description, price_ngn, price_usd, tags)
SELECT * FROM (VALUES
...
) AS new_recommendations(name, category, description, price_ngn, price_usd, tags)
WHERE NOT EXISTS (
  SELECT 1 FROM recommendations WHERE name = new_recommendations.name
);